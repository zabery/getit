#ifndef __GETIT_COMMON_SPELLHELPER_H__
#define __GETIT_COMMON_SPELLHELPER_H__

#include "stdafx.h"
namespace KyUtility
{
	inline
		std::wstring ConvertStringToWstring(const std::string& s, UINT nCodePage = CP_ACP)
	{
		int len;
		int slength = (int)s.length() + 1;
		len = MultiByteToWideChar(nCodePage, 0, s.c_str(), slength, 0, 0); 
		wchar_t* buf = new wchar_t[len];
		MultiByteToWideChar(nCodePage, 0, s.c_str(), slength, buf, len);
		std::wstring r(buf);
		delete[] buf;
		return r;
	}
	...
	inline
		std::wstring ConvertChineseToPinYin(wchar_t* chrstr)
	{
		...
		std::wstring sPinYin = ConvertStringToWstring(pcReturnString);
